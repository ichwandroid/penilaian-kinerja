...
$ekstensi   = array('pdf');
$filename   = $_FILES['filesmp']['name'];
$ukuran     = $_FILES['filesmp']['size'];
$temp       = $_FILES['filesmp']['tmp_name'];
...
if (!in_array($ext, $ekstensi)) {
    ...
    if ($ukuran < 1044070) {
        ...
        mysqli_query($koneksi, "UPDATE study SET namasmp='$nama_smp', ijasahsmp='$file' WHERE id_study='$id_user'");
...
//             $get_foto = "SELECT ijasahsmp FROM study WHERE id_study='$id'";
//             $data_foto = mysqli_query($koneksi, $get_foto); 
//             // Mengubah data yang diambil menjadi Array
//             $foto_lama = mysqli_fetch_array($data_foto);

//             // Menghapus Foto lama didalam folder FOTO
//             unlink("../ijasah/".$foto_lama['ijasahsmp']);    

//             // Memindahkan File kedalam Folder "FOTO"
//             move_uploaded_file($file_tmp, '../ijasah/'.$foto_nama_new);

//             // Query untuk memasukan data kedalam table SISWA
//             $query = mysqli_query($koneksi, "UPDATE study SET ijasahsmp='$foto_nama_new' WHERE id_study='$id'");
