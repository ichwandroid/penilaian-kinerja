...
if (isset($_POST['id'])) {
    ...
    if ($foto_size > 2097152) {
       ...
       if ($foto_nama != "") {
          ...
          $foto_nama_new = md5('ijasah sma -'.$id); 
          ...
          if(in_array($ekstensi, $ekstensi_izin) === true)  {
            ...
            $get_foto = "SELECT ijasahsma FROM study WHERE id_study='$id'";
            $data_foto = mysqli_query($koneksi, $get_foto); 
            // Mengubah data yang diambil menjadi Array
            $foto_lama = mysqli_fetch_array($data_foto);

            // Menghapus Foto lama didalam folder FOTO
            unlink("../ijasah/".$foto_lama['ijasahsma']);    

            // Memindahkan File kedalam Folder "FOTO"
            move_uploaded_file($file_tmp, '../ijasah/'.$foto_nama_new);

            // Query untuk memasukan data kedalam table SISWA
            $query = mysqli_query($koneksi, "UPDATE study SET ijasahsma='$foto_nama_new' WHERE id_study='$id'");
