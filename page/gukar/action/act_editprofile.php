...
if (isset($_POST['id'])) {
    ...
    if ($foto_size > 2097152) {
       ...
       header("location:../profile.php?pesan=size");
       ...
       if ($foto_nama != "") {
          ...
          $foto_nama_new = $tanggal.'-'.$namalengkap; 
          ...
          if(in_array($ekstensi, $ekstensi_izin) === true)  {
            ...
            $get_foto = "SELECT gukar_foto FROM user WHERE id_user='$id'";
            $data_foto = mysqli_query($koneksi, $get_foto); 
            // Mengubah data yang diambil menjadi Array
            $foto_lama = mysqli_fetch_array($data_foto);

            // Menghapus Foto lama didalam folder FOTO
            unlink("../foto/".$foto_lama['gukar_foto']);    

            // Memindahkan File kedalam Folder "FOTO"
            move_uploaded_file($file_tmp, '../foto/'.$foto_nama_new);

            // Query untuk memasukan data kedalam table SISWA
            $query = mysqli_query($koneksi, "UPDATE user SET nama='$namalengkap', jeniskelamin='$jk', alamat='$alamat', tempat_lahir='$tempatlahir', tanggal_lahir='$tanggallahir', whatsapp='$whatsapp', gukar_foto='$foto_nama_new' WHERE id_user='$id'");
            ...
            header("location:../profile.php?pesan=ekstensi");        }

        }else{

        // Apabila tidak ada file yang diupload maka akan menjalankan code dibawah ini
          $query = mysqli_query($koneksi, "UPDATE user SET nama='$namalengkap', jeniskelamin='$jk', alamat='$alamat', tempat_lahir='$tempatlahir', tanggal_lahir='$tanggallahir', whatsapp='$whatsapp' WHERE id_user='$id'");
